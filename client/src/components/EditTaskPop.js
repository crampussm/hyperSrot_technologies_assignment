...
function EditTaskPop() {
  const { theme, popEditTask, selectedTask, updateTask } = GetContext();
  const [priorityDropdown, setPriorityDropdown] = useState(false);
    const [statusDropdown, setStatusDropdown] = useState(false);
    const [taskDetails, setTaskDetails] = useState({priority: selectedTask.priority, status: selectedTask.status});
  ...
  return (
      <div className={`z-10 absolute top-16 right-96 w-[40%] rounded-md flex flex-col items-center ${theme === 'dark' ? 'bg-tmpBlack' : 'bg-blue-50'}`} style={{boxShadow: "rgba(0, 0, 0, 0.25) 0px 0.0625em 0.0625em, rgba(0, 0, 0, 0.25) 0px 0.125em 0.5em, rgba(255, 255, 255, 0.1) 0px 0px 0px 1px inset"}}>
          <div className='w-full flex justify-end p-2'>
              <Plus className={`rotate-45 text-gray-50 cursor-pointer ${theme === 'dark' ? 'text-gray-50' : 'text-gray-600'}`} onClick={popEditTask}/>
          </div>
          <form action="" className='w-full px-20 py-14' onSubmit={callEditTask}>
              <div className='flex flex-col mt-2'>
                  ...
                  <input type="text" id='spacename-create' className={` px-2 py-1 rounded-md font-tmpfont border-2 ${theme === 'dark' ? 'bg-tmplightBlack text-gray-400 border-gray-600' : 'bg-white text-gray-600 border-gray-300'}`} name='taskname' value={selectedTask.taskname} readOnly/>
              </div>
              <div className='flex flex-col mt-2'>
                  ...
                  <input type="text" id='spacename-create' className={` px-2 py-1 rounded-md font-tmpfont border-2 ${theme === 'dark' ? 'bg-tmplightBlack text-gray-400 border-gray-600' : 'bg-white text-gray-600 border-gray-300'}`} name='description' value={selectedTask.description} readOnly/>
              </div>
              <div className='flex flex-col mt-2'>
                  ...
                  <input type="text" id='spacename-create' className={` px-2 py-1 rounded-md font-tmpfont border-2 ${theme === 'dark' ? 'bg-tmplightBlack text-gray-400 border-gray-600' : 'bg-white text-gray-600 border-gray-300'}`} name='assignees' value={selectedTask.assignees} readOnly/>
              </div>
                      ...
                      </span>
                      {
                          ...
                          <div className={`absolute rounded-md w-[20%] py-1 bottom-24 ${theme === 'dark' ? 'bg-tmplightBlack text-gray-50' : 'bg-blue-200 text-gray-800'}`}>
                              <ul className='w-full'>
                                  ...
                                  <li className={`cursor-pointer w-full flex justify-center px-4 ${theme === 'dark' ? 'hover:bg-gray-700' : 'hover:bg-blue-300'}`} onClick={e => setStatus('Deffered')}>Deferred</li>
                              </ul>
                          </div>
                      }
                  </div>
              ...
              <div className='flex flex-col mt-4'>
                  ...
                  <input type="date" className={`cursor-pointer px-2 py-1 rounded-md font-tmpfont border-2 ${theme === 'dark' ? 'bg-tmplightBlack text-gray-400 border-gray-600' : 'bg-white text-gray-600 border-gray-300'}`} name='endDate' value={selectedTask.endDate} readOnly/>
              </div>
              <div className='flex justify-center items-center mt-10'>
                  <button className='text-white bg-tmpBlue px-6 py-2 rounded-3xl font-tmpfont'>Edit</button>
              </div>
          </form>
      </div>
  )
